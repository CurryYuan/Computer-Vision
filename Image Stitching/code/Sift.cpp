...
Sift::Sift(const char* _filename, int _isColor) {
	filename = _filename;
	isColor = _isColor;
}

//下采样原来的图像，返回缩小2倍尺寸的图像  
CvMat * Sift::halfSizeImage(CvMat * im) {
	unsigned int i, j;
	int w = im->cols / 2;
	int h = im->rows / 2;
...
CvMat * Sift::doubleSizeImage(CvMat * im) {
	unsigned int i, j;
	int w = im->cols * 2;
	int h = im->rows * 2;
	CvMat *imnew = cvCreateMat(h, w, CV_32FC1);
...
CvMat * Sift::doubleSizeImage2(CvMat * im) {
	unsigned int i, j;
	int w = im->cols * 2;
	int h = im->rows * 2;
	CvMat *imnew = cvCreateMat(h, w, CV_32FC1);
...
//双线性插值，返回像素间的灰度值  
float Sift::getPixelBI(CvMat * im, float col, float row) {
	int irow, icol;
	float rfrac, cfrac;
	float row1 = 0, row2 = 0;
	int width = im->cols;
...
//矩阵归一化  
void Sift::normalizeMat(CvMat* mat)
{
...
//向量归一化  
void Sift::normalizeVec(float* vec, int dim)
{
	unsigned int i;
	float sum = 0;
	for (i = 0; i < dim; i++)
...
//产生1D高斯核  
float* Sift::GaussianKernel1D(float sigma, int dim)
{
...
//产生2D高斯核矩阵  
CvMat* Sift::GaussianKernel2D(float sigma)
{
...
//x方向像素处作卷积  
float Sift::ConvolveLocWidth(float* kernel, int dim, CvMat * src, int x, int y)
{
...
//x方向作卷积  
void Sift::Convolve1DWidth(float* kern, int dim, CvMat * src, CvMat * dst)
{
...
//y方向像素处作卷积  
float Sift::ConvolveLocHeight(float* kernel, int dim, CvMat * src, int x, int y)
{
#define Src(ROW,COL) ((float *)(src->data.fl + src->step/sizeof(float) *(ROW)))[(COL)]  
	unsigned int j;
	float pixel = 0;
...
//y方向作卷积  
void Sift::Convolve1DHeight(float* kern, int dim, CvMat * src, CvMat * dst)
{
#define Dst(ROW,COL) ((float *)(dst->data.fl + dst->step/sizeof(float) *(ROW)))[(COL)]  
	unsigned int i, j;
	for (j = 0; j < src->rows; j++)
...
//卷积模糊图像  
int Sift::BlurImage(CvMat * src, CvMat * dst, float sigma)
{
	float* convkernel;
	int dim = (int)max(3.0f, 2.0 * GAUSSKERN * sigma + 1.0f);
	CvMat *tempMat;
...
CvMat * Sift::ScaleInitImage(CvMat * im)
{
...
//SIFT算法第二步  
ImageOctaves* Sift::BuildGaussianOctaves(CvMat * image)
{
...
//SIFT算法第三步，特征点位置检测，  
int Sift::DetectKeypoint(int numoctaves, ImageOctaves *GaussianPyr) {
...
//在图像中，显示SIFT特征点的位置  
void Sift::DisplayKeypointLocation(IplImage* image, ImageOctaves *GaussianPyr) {
...
void Sift::ComputeGrad_DirecandMag(int numoctaves, ImageOctaves *GaussianPyr)
{
...
void Sift::AssignTheMainOrientation(int numoctaves, ImageOctaves *GaussianPyr, ImageOctaves *mag_pyr, ImageOctaves *grad_pyr)
{
...
int Sift::FindClosestRotationBin(int binCount, float angle)
{
	angle += CV_PI;
	angle /= 2.0 * CV_PI;
...
void Sift::AverageWeakBins(double* hist, int binCount)
{
...
bool Sift::InterpolateOrientation(double left, double middle, double right, double *degreeCorrection, double *peakValue)
{
...
//显示特征点处的主方向  
void Sift::DisplayOrientation(IplImage* image, ImageOctaves *GaussianPyr)
{
...
void Sift::ExtractFeatureDescriptors(int numoctaves, ImageOctaves *GaussianPyr) {
...
//为了显示图象金字塔，而作的图像水平拼接  
CvMat* Sift::MosaicHorizen(CvMat* im1, CvMat* im2)
{
	int row, col;
...
//为了显示图象金字塔，而作的图像垂直拼接  
CvMat* Sift::MosaicVertical(CvMat* im1, CvMat* im2) {
	int row, col;
...
int Sift::getKeyPointsCount() {
	return keypoint_count;
}

Keypoint Sift::getFirstKeyDescriptors() {
	return keyDescriptors;
}

void Sift::saveImgWithKeypoint(const char* filename) {
