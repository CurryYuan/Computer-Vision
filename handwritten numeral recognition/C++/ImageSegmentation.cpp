...
void ImageSegmentation::processBinaryImage() {
	...
	auto res = SrcGrayImg;
	cimg_foroff(SrcGrayImg, off) {
		if (off > 0) {
			int temp = off - SrcGrayImg.width() / 16;
			temp = temp > 0 ? temp : 0;
			int sum = 0;
			for (int i = temp; i < off; ++i) {
				sum += SrcGrayImg[i];
			}
			sum /= (off - temp);
			if (SrcGrayImg[off] > sum*0.9) {
				res[off] = 255;
			}
			else {
				res[off] = 0;
	...
	CImg<uchar> res1(res.width() - 50, res.height() - 25, 1, 1);
	cimg_forXY(res1, x, y) {
		res1(x, y) = res(x + 25, y + 25);
	}
	BinaryImg = res1;
}
...
vector<int> getInflectionPosXs(const CImg<int>& XHistogramImage) {
	...
	if (distNum == 0 || tempInflectionPosXs.size() < 3)
		return resultInflectionPosXs;

	for (int i = 2; i < tempInflectionPosXs.size() - 1; i = i + 2) {
		cout << tempInflectionPosXs.size() - 1 << endl;
...
void ImageSegmentation::saveSingleNumberImageAndImglist(int barItemIndex) {
	...
	for (int i = 0; i < pointPosListSet.size(); i++) {
		...
		if (pointPosListSet[i].size() != 0 && pointPosListSet[i].size() > avgBlacks * NumberImageBlackPixelPercentage) {
			...
			string tempAddr = basePath + to_string(barItemIndex);
			if (_access(tempAddr.c_str(), 0) == -1)
				_mkdir(tempAddr.c_str());
			tempAddr = tempAddr + "/";


			char addr[200];
			sprintf(addr, "%s%d%s", tempAddr.c_str(), classTagSet[i], postfix.c_str());
			singleNum.save(addr);
